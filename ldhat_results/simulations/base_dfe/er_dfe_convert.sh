...
# Usage:
#	./er_dfe_convert.sh [modified_fasta_file]
...
stashcp /osgconnect/public/sjsabin/er_dfe_sub_slim.tar.gz ./
# Extract the archive of fastas
tar xvf er_dfe_sub_slim.tar.gz
...
gunzip er_dfe_sub_slim/$1
# Run LDhat convert on the modified fasta file
LDhat-master/convert -seq er_dfe_sub_slim/${FILE} -prefix ${REP}. > ${REP}.convert.log
# Remove the large input file from the host directory
rm -r er_dfe_sub_slim/
