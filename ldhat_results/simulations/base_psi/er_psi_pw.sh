...
#stashcp /osgconnect/public/sjsabin/er_psi_convert_out.tar.gz ./
# Extract the fasta files
tar xf er_psi_convert_out.tar.gz

REP=er_psi.$1

echo "0.000058 100 201 0 0 1 1 1" | LDhat-master/pairwise -seq er_psi_convert_out/${REP}.sites.txt -loc er_psi_convert_out/${REP}.locs.txt -prefix ${REP}. >> ${REP}.pw.log

# Remove the large input file from the host directory
rm -r er_psi_convert_out/
rm -r LDhat-master/